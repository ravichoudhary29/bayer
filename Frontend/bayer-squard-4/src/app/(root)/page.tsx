    ...
    <div>
      ...
      >
        <div className="container mx-auto px-4">
          <h3 id="featured-topics-heading" className="text-2xl font-bold text-gray-800 mb-8">
            Featured Health Topics
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {healthTopics.map((topic, index) => (
              <HealthTopicCard key={index} title={topic.title} description={topic.description} />
            ))}
          </div>
        </div>
      </section>
